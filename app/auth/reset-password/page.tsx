...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    const checkRecoverySession = async () => {
      console.log("[v0] Checking for recovery session")
      ...
      console.log("[v0] Hash params:", { accessToken: !!accessToken, refreshToken: !!refreshToken, type })

      if (accessToken && type === "recovery") {
        console.log("[v0] Recovery tokens found in hash, setting session")
        try {
          ...
          console.error("[v0] Exception setting session:", err)
          setError("Failed to validate reset link. Please request a new one.")
          setCheckingSession(false)
          return
        }
      ...
      if (sessionError) {
        ...
        setError("Failed to validate session. Please request a new reset link.")
        setCheckingSession(false)
        return
      }
      ...
      if (!session) {
        ...
        console.log("[v0] Valid session found for user:", session.user.id)
        setCheckingSession(false)
      }
    }
